...
angular.module('FlipCounter', [])
  .directive('flipcounter', function () {
    return {
      restrict: 'A',
      template: '<div class="counter-wrapper">' +
                  '<ul class="flip-counter default" id="TeamBCounter"></ul>'+
                '</div>',
      scope: {
        scoreValue: '='
      },        
...
var flipCounter = function(d, options){
  ...
  function _toArray(input){
     var value = input.toString().split('').reverse();
     if(value.length < 2){
      value.push(0);
     }
     return value;
